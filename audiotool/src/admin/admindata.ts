...
export class AdminData {
  ...
  async update(): Promise<void> {
    await this.run(["users", "tasksets", "consents"], async () => {
      ...
      for (const user of (await (
        await authenticatedFetch("/api/admin/listusers")
      ).json()) as schema.EUserInfo[]) {
        this.users.set(user.euid, user);
      }
      for (const ts of (await (
        await authenticatedFetch("/api/admin/listtasksets")
      ).json()) as schema.ETaskSetInfo[]) {
        this.tasksets.set(ts.id, ts);
      }
      for (const c of (await (
        await authenticatedFetch("/api/admin/listconsents")
      ).json()) as schema.EConsentInfo[]) {
  ...
  private async run<X>(
    ...
    return await Spinner.waitFor(async () => {
      ...
      if (
        changes.indexOf("users") !== -1 ||
        changes.indexOf("tasksets") !== -1 ||
        changes.indexOf("consents") !== -1
      ) {
  ...
  async addTaskSet(id: string, name: string, language: string) {
    await this.run(["tasksets"], async () => {
      const [ts] = (await postAsJson("/api/admin/newtaskset", {
        ...
        name,
        language,
      })) as [schema.ETaskSetInfo];
      this.tasksets.set(ts.id, ts);
    });
  }

  // Adds an enrollment rule to a task set
  async addTaskSetRule(
    taskSetId: string,
    id: number,
    order: number,
    tags: string[],
  ...
  private async editTaskSet(info: {
    taskSetId: string;
    delrules: number[];
    addrules: schema.EAssignmentRule[];
    name?: string;
    language?: string;
  }): Promise<void> {
    await this.run(["tasksets"], async () => {
  ...
  async loadUserWork(
    ...
    const rv = await this.run(["users"], async () => {
      ...
      this.users.set(user.euid, user);
      const result: [schema.EUserTaskInfo[], schema.ERecordingMetadata[]] = [
        tasks,
        recordings,
      ];
      return result;
  ...
  async addConsent(
    ...
    await this.run(["consents"], async () => {
      const [consent] = (await postAsJson("/api/admin/newconsent", {
        ...
        tags,
        optional,
      })) as [schema.EConsentInfo];
      this.consents.set(consent.id, consent);
    });
  }

  // Changes a consent's metadata
  async editConsentInfo(
    id: string,
    name: string,
    language: string,
    tags: string[],
