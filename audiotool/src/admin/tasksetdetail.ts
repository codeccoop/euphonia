...
export class TaskSetDetailView {
  ...
  private fillTaskSetInfo() {
    ...
    const desc = this.tsinfo
      .eadd("<div class=description />")
      .etext(`${this.taskset.name}`);
    desc.eadd("<span class=language />").etext(` (${this.taskset.language})`);
    desc
      .eadd("<a class=editlink />")
      .etext("[edit]")
      .on("click", async (e) => await this.startEdit());
...
class AddImageTaskDialog extends Dialog {
  ...
  constructor(parent: TaskSetDetailView) {
    ...
    buttonTd
      .eadd("<button>Upload Image Task</button>")
      .on("click", async (e) => {
        const prompt = promptField.val() as string;
...
class AddRuleDialog extends Dialog {
  ...
  constructor(parent: TaskSetDetailView) {
    ...
    choices
      .eadd("<label for=allradio />")
      .text("Assign all tasks from this taskset");
    choices.eadd("<br />");
    this.sampleChoice = choices.eadd(
      "<input type=radio class=radio name=action value=sample id=sampleradio />",
    );
    choices
      .eadd("<label for=sampleradio />")
      .text("Assign a random subset from this taskset");
    ...
    this.sampleChoice.on("change", (e) =>
      this.sampleSize.eenable(this.sampleChoice.is(":checked")),
    );
    this.allChoice.on("change", (e) =>
      this.sampleSize.eenable(this.sampleChoice.is(":checked")),
    );
    ...
    buttonTd
      .eadd("<button>Create Rule</button>")
      .on("click", async (e) => await this.handleAddRule());
    buttonTd
      .eadd("<button>Cancel</button>")
      .on("click", async (e) => await this.remove());
  }
  ...
  private async handleAddRule() {
    ...
    const action = this.allChoice.is(":checked")
      ? "all"
      : this.sampleChoice.is(":checked")
        ? "sample"
        : "";
...
class EditTasksetDialog extends Dialog {
  ...
  constructor(parent: TaskSetDetailView) {
    ...
    this.parent = parent;
    this.div
      .eadd("<div class=title />")
      .text(`Edit Taskset: ${parent.taskset.id}`);
    this.startForm();
