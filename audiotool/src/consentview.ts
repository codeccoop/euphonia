...
export class ConsentView {
  ...
  constructor(app: App) {
    ...
    this.div.eadd("<div class=wholine />")
      .eihtml(`You are enrolling as <b id=whoisenrolling>&nbsp;</b>.
  ...
  private displayError() {
    ...
    this.consentDiv.eitext(
      "Unfortunately enrollment is not configured. Please check the URL or contact your program administrator.",
    );
  }
  ...
  private hasAgreement(consent: schema.EConsentInfo): boolean {
    ...
    if (consent.versions.length !== 1) {
      throw new Error(
        `Unexpected multiple live versions for consent: ${consent.id}`,
      );
    }
    for (const agreement of this.data.user.consents) {
      if (
        agreement.consentId === consent.id &&
  ...
  private async displayConsentIdx(idx: number) {
    ...
    if (consent.versions.length !== 1) {
      throw new Error(
        `Unexpected applicable versions for consent: ${consent.id}: ${consent.versions.length} versions`,
      );
    }
    if (idx !== 0 || !isLast) {
      $("#consentcounter").eihtml(
        `&nbsp;(Agreement {which_agreement_number} of {total_number_of_agreements})`,
        "which_agreement_number",
        `${idx + 1}`,
        "total_number_of_agreements",
        `${consentCount}`,
      );
    }

    const consentText = await this.data.loadConsentText(
      consent.id,
      consent.versions[0].version,
    );
    ...
    nextButton.eitext(
      isLast ? (isAgreed ? "Continue" : "Enroll") : "Next Agreement",
    );
    nextButton.on("click", async (e) => {
      ...
      if (!consent.optional && !sigText.trim()) {
        ...
        this.app.showMessage(
          "You must type your name to agree to the terms.",
          "error",
        );
        return;
      }
