 ...
 */
define(["require", "exports", "./userdetail", "./tasksets", "../../commonsrc/util", "../../commonsrc/userfilter", "../util"], function (require, exports, userdetail_1, tasksets_1, util_1, userfilter_1, util_2) {
    "use strict";
    ...
    class UsersView {
        constructor(app) {
            this.sortSpec = new Set(['Last Recorded']); // list of labels to sort by, from least to most significant
            this.filterFn = user => true; // predicate that returns false if the given user should be filtered away
            this.app = app;
        ...
        // Refills the table with the current list of users
        drawTable() {
            const currentSort = (0, util_1.lastitem)(this.sortSpec);
            ...
            for (const user of this.toSorted(this.app.data.users)) {
                ...
                tr.eadd('<td class=date />').text((0, util_1.formatTimestamp)(user.lastRecordingTimestamp));
                ...
                tr.eadd('<td class=date />').text((0, util_1.formatTimestamp)(user.createTimestamp));
                tr.eadd('<td class=date />').text((0, util_1.formatTimestamp)(user.signupTimestamp, 'Never'));
            }
        }
        ...
        setFilter(filterSpec) {
            if (!filterSpec || filterSpec.trim() == '') {
                ...
                this.filterFn = userfilter_1.UserFilter.parse(filterSpec);
            }
            this.drawTable();
        }
        ...
        setSort(sortByHeader) {
            ...
            const lastSort = (0, util_1.lastitem)(this.sortSpec);
        ...
        async startNewUser() {
            const defaults = new userdetail_1.UserValues();
        ...
        async startBulkAssign() {
            ...
            if (euids.length === 0) {
                (0, util_2.toast)('No users selected');
                return;
            }
            await new tasksets_1.BulkAssignDialog(this.app, euids).start();
        }
        async startUserDetail(euid) {
            const user = this.app.data.users.get(euid);
            if (!user) {
                (0, util_2.toast)(`No such user: ${euid}`);
                return;
            }
            this.userdetail = new userdetail_1.UserDetailView(this, user);
...
`;
    // Creates a new user
    class AddUserDialog extends userdetail_1.UserPropertiesDialog {
