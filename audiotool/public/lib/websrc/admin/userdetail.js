...
define(["require", "exports", "../util", "../dialog", "../../commonsrc/util", "./tasksets", "../../commonsrc/schema"], function (require, exports, util_1, dialog_1, util_2, tasksets_1, schema) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.UserPropertiesDialog = exports.UserValues = exports.UserDetailView = void 0;
    ...
    class UserDetailView {
        ...
        fillUserInfo() {
            ...
            table.eaddtr([$('<span class=label />').etext('Signed up:'), $('<span />').etext(`${(0, util_2.formatTimestamp)(this.user.signupTimestamp, 'never')}`)]);
            table.eaddtr([$('<span class=label />').etext('Last recorded:'), $('<span />').etext(`${(0, util_2.formatTimestamp)(this.user.lastRecordingTimestamp)}`)]);
            ...
            for (const consent of this.user.consents) {
                ...
                const cts = (0, util_2.formatTimestamp)(consent.consentTimestamp);
        ...
        async onUserTasksChanged() {
            await util_1.Spinner.waitFor(async () => {
                ...
                for (const task of this.tasks) {
                    ...
                    tr.eadd('<td class=assigned />').text((0, util_2.formatTimestamp)(task.assignedTimestamp));
                    tr.eadd('<td class=completed />').text((0, util_2.formatTimestamp)(task.recordedTimestamp));
        ...
        async unassignAll() {
            const confirm = await dialog_1.ChoiceDialog.choose('Unassigning all unrecorded tasks?', 'Unassign', 'Cancel');
            if (confirm !== 'Unassign') {
                return;
            }
            const tasks = this.tasks.filter(task => task.recordedTimestamp === 0);
            if (tasks.length === 0) {
                (0, util_1.toast)('No unrecorded tasks');
                return;
            }
            await util_1.Spinner.waitFor(async () => {
        ...
        async startAssign() {
            await new tasksets_1.BulkAssignDialog(this.parent.app, [this.user.euid]).start();
        }
        // Deletes the user and all their recordings, after confirmation
        async deleteUser() {
            const confirm = await dialog_1.ChoiceDialog.choose('Delete user and all recordings? This cannot be undone.', 'DELETE', 'Cancel');
            if (confirm !== 'DELETE') {
                return;
            }
            await util_1.Spinner.waitFor(async () => {
    ...
    exports.UserValues = UserValues;
    // Abstract base class for new user / edit user dialog
    class UserPropertiesDialog extends dialog_1.Dialog {
    ...
    exports.UserPropertiesDialog = UserPropertiesDialog;
