...
define(["require", "exports", "./util", "./progresswidget", "../commonsrc/schema", "../commonsrc/util"], function (require, exports, util_1, progresswidget_1, schema, util_2) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.RecordingView = void 0;
    schema = __importStar(schema);
    // Suggest that the user wait before speaking
    const MIN_START_DELAY_MS = (0, util_1.isSafari)() ? 750 : 250;
    const MIN_STOP_DELAY_MS = (0, util_1.isSafari)() ? 750 : 250;
    // Lets the user record cards
    class RecordingView {
        constructor(app) {
            // GUI visibility tracking vs. other app views
            this.isShown = false;
            this.seenRecording = false; // used to skip instructions
            // Recording state tracking
            this.isRecording = false; // the microphone is active
            this.isStartingRecord = false; // the browser is setting up the record pipeline; this takes 700ms on safari
            this.isStoppingRecord = false; // we just finished recording and are uploading or canceling
            this.isUploadingNew = false; // the current upload is for new audio, not replacing a prior recording
            this.isCanceling = false; // the user canceled the recording and we're waiting for Recorder to stop
            this.isDeleting = false; // the user deleted the recording and we're waiting for the server to do it
            this.recordedWav = new util_2.WavBuilder();
            this.app = app;
            this.data = app.data;
            ...
            new util_1.Swiper(this.cardDiv, async (s) => await this.handleSwipe(s));
            ...
            this.progressBar = new progresswidget_1.ProgressWidget(statusBox);
        ...
        updateGUI() {
            ...
            this.listenButton.eshow(showRecordedCardControls && !((0, util_1.isSafari)() && this.replayer));
            ...
            if (this.task) {
                ...
                if (isImageTask) {
                    ...
                    this.cardDiv.eadd('<div class=image />').css('background-image', `url(${(0, util_1.toURL)('/api/gettaskimage', args)})`);
                }
            ...
            if (user) {
                ...
                if (isDone) {
                    ...
                    this.progressBar.setEIHtml(`<b>{number_of_completed_cards}</b> of <b>{total_number_of_tasks_needed}</b> cards <b>done</b>`, 'number_of_completed_cards', `${user.numCompletedTasks}`, 'total_number_of_tasks_needed', `${user.numTasks}`);
                }
        ...
        findTask(fromTask, step, unrecorded = false) {
        ...
        async toggleRecord(wantUpload = true) {
        ...
        async startRecording() {
            ...
            this.recordedWav = new util_2.WavBuilder();
            this.setupProcessor();
            await (0, util_1.sleep)(startTime + MIN_START_DELAY_MS - Date.now());
        ...
        async getAudioStream() {
            ...
            if (deviceId == '') {
                ...
                options['audio'] = (0, util_1.isSafari)() ? { echoCancellation: false } : true;
            }
            else {
        ...
        async toggleListen() {
            ...
            this.replayer = this.secondaryControls.eins(`<audio controls src="${url}" />`);
            if (!(0, util_1.isSafari)()) {
        ...
        async getPlaybackURL(task) {
            return await util_1.Spinner.waitFor(async () => {
                const rsp = await (0, util_1.authenticatedFetch)('/api/getaudio', { ts: task.recordedTimestamp });
                let contentType = rsp.headers.get('Content-Type');
                if (!contentType || contentType == 'application/octet-stream') {
                    contentType = 'audio/wav'; // here's hoping!
                }
                const b64String = (0, util_2.toBase64)(await rsp.arrayBuffer());
        ...
        async stopRecording(wantUpload) {
            const stopTime = Date.now();
            this.isCanceling = !wantUpload;
            this.isStoppingRecord = true;
            this.updateGUI();
            this.deleteProcessor();
            await (0, util_1.sleep)(stopTime + MIN_STOP_DELAY_MS - Date.now());
            let uploaded = false;
            let canceled = false;
            let success = false;
            try {
    ...
    exports.RecordingView = RecordingView;
});
