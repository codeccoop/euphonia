...
export class SetupView {
  ...
  updateGUI() {
    ...
    this.doneButton.eitext(
      hasRecordings ? "Continue recording!" : "Start recording!",
    );
  }
...
class MicrophoneSettingsPanel {
  ...
  private addDefaultCheckbox() {
    ...
    this.defaultCheckbox = defaultrow.eadd(
      `<input type=checkbox id="defaultaudiodevice" />`,
    );
    defaultrow
      .eadd(`<label for="defaultaudiodevice" />`)
      .eitext("Use the default microphone");
  ...
  private addDeviceList() {
    ...
    for (const device of this.devices) {
      ...
      const radio = buttonrow.eadd(
        `<input name=micdevice type=radio id="${deviceHtmlId}" />`,
      );
    ...
    if (this.defaultCheckbox!.is(":checked")) {
      ...
      for (const htmlId of this.deviceCheckboxIds.keys()) {
        if ($(`#${htmlId}`).is(":checked")) {
          this.data.saveMicrophoneChoice(
            this.deviceCheckboxIds.get(htmlId)!.deviceId,
          );
          break;
        }
      }
  ...
  private async queryDevices(): Promise<void> {
    ...
    this.hasDefault = !!allDevices.filter(
