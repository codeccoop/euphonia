...
export class App implements Listener {
  ...
  constructor() {
    ...
    this.messageBox = this.main.eadd(
      '<div id=messagebox style="opacity: 0;" />',
    );
    ...
    // Events
    $(window).on("hashchange", async () => await this.handleHashChange());
  ...
  private parseHash(): void {
    ...
    const parts =
      hash && hash.startsWith("#") ? hash.substring(1).split(";") : [];
  ...
  private chooseBestNav(): void {
    ...
    if (!this.data.fbuser || !this.signupView.eligible) {
      ...
      this.data.user.numRecordings === 0 &&
      !this.recordingView.seenRecording
    ) {
