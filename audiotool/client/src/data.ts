...
import { formatDateCode } from "../../common/util";
...
export class Data {
  ...
  constructor(listener: Listener) {
    ...
    firebase
      .auth()
      .onAuthStateChanged(
        async (user: FBUser) => await this.handleUserAuth(user),
      );
  }
  ...
  async handleUserAuth(fbuser: FBUser): Promise<void> {
    ...
    if (this.fbuser == null) {
      ...
      const [euser, etasks, isConsented] = result as [
        schema.EUserInfo,
  ...
  async listConsents(
    language: string,
    tags: string[],
  ): Promise<schema.EConsentInfo[]> {
    return (await postAsJson("/api/listconsents", {
      language,
      tags,
  ...
  async enroll(
    ...
    demographics: schema.UserDemographics,
  ): Promise<void> {
    if (!this.fbuser) {
      throw new Error("Unexpected signup for unauthenticated user");
    }
    const result = await postAsJson("/api/signup", {
      language,
      tags,
      agreements,
      demographics,
  ...
  async updateAgreements(agreements: schema.EAgreementInfo[]): Promise<void> {
    if (!this.fbuser || !this.user) {
      throw new Error(
        "Unexpected agreement for unauthenticated/unenrolled user",
      );
    }
  ...
  async saveAudio(
    ...
    return await Spinner.waitFor(async () => {
      ...
      const [euser, etask, erec] = (await rsp.json()) as [
        schema.EUserInfo,
        schema.EUserTaskInfo,
  ...
  async deleteAudio(task: schema.EUserTaskInfo) {
    await Spinner.waitFor(async () => {
      ...
      const [euser, etask] = (await rsp.json()) as [
        schema.EUserInfo,
        schema.EUserTaskInfo,
      ];
      await this.updateTask(euser, etask);
  ...
  private async checkPermissions() {
    try {
      ...
      console.log(
        `Failed to query microphone permission, assuming we'll have to ask.`,
      );
      console.error(e);
      this.hasMicrophonePermission = "maybe";
    }
