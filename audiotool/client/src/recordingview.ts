...
export class RecordingView {
  ...
  constructor(app: App) {
    ...
    this.ttsButton = this.cardRibbon
      .eadd("<button class=tts />")
      .eihtml("&#x1F50A;");
    ...
    this.listenButton = this.secondaryControls
      .eadd("<button class=listen />")
      .eitext("Replay");
    this.deleteButton = this.secondaryControls
      .eadd("<button class=delete />")
      .eitext("Delete");
    this.recordButton = mainControls
      .eadd("<button class=record />")
      .eitext("Record");
  ...
  private updateGUI() {
    ...
    const isOldRecording =
      isRecorded && this.task!.recordedTimestamp < editDeadline;
  ...
  private findTask(
    ...
    const isValidTask =
      fromTask &&
      this.data.tasksById.get(fromTask.id) &&
      this.taskOrder.indexOf(fromTask.id) != -1;
    if (!isValidTask) {
