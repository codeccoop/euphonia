...
export class TaskSetsView {
  ...
  onDataChanged() {
    ...
    for (const [, ts] of this.app.data.tasksets) {
      ...
      tr.eadd("<td class=tsid />")
        .eadd(`<a href="#/taskset/${ts.id}" />`)
        .etext(ts.id);
...
export class BulkAssignDialog extends Dialog {
  ...
  constructor(app: AdminView, euids: string[]) {
    ...
    choicetd1
      ...
      .text("All tasks from this taskset");
    ...
    buttonTd
      .eadd("<button>Assign</button>")
      .on("click", async (e) => await this.handleAssign());
    buttonTd
  ...
  private async loadTaskSet(tsid: string) {
    ...
    await Spinner.waitFor(async () => {
      ...
      for (const task of tasks) {
        ...
        const tick = tr
          .eadd("<td class=tick />")
          .eadd("<input type=checkbox class=tick />");
