...
import { formatTimestamp, lastitem } from "../../../common/util";
import { UserFilter, UserPredicate } from "../../../common/userfilter";
...
export class UsersView {
  ...
  static SORT_COMPARATORS = {
    EUID: (a: schema.EUserInfo, b: schema.EUserInfo) =>
      a.euid.localeCompare(b.euid),
    Name: (a: schema.EUserInfo, b: schema.EUserInfo) =>
      a.name.localeCompare(b.name),
    Email: (a: schema.EUserInfo, b: schema.EUserInfo) =>
      a.email.localeCompare(b.email),
    Language: (a: schema.EUserInfo, b: schema.EUserInfo) =>
      a.language.localeCompare(b.language),
    Tags: (a: schema.EUserInfo, b: schema.EUserInfo) =>
      JSON.stringify(a.tags).localeCompare(JSON.stringify(b.tags)),
    "Last Recorded": (a: schema.EUserInfo, b: schema.EUserInfo) =>
      b.lastRecordingTimestamp - a.lastRecordingTimestamp,
    "Num Recordings": (a: schema.EUserInfo, b: schema.EUserInfo) =>
      a.numRecordings - b.numRecordings,
    "Total Tasks": (a: schema.EUserInfo, b: schema.EUserInfo) =>
      a.numTasks - b.numTasks,
    "Tasks Completed": (a: schema.EUserInfo, b: schema.EUserInfo) =>
      a.numCompletedTasks - b.numCompletedTasks,
    "Tasks Remaining": (a: schema.EUserInfo, b: schema.EUserInfo) =>
      a.numTasks - a.numCompletedTasks - (b.numTasks - b.numCompletedTasks),
    Created: (a: schema.EUserInfo, b: schema.EUserInfo) =>
      b.createTimestamp - a.createTimestamp,
    "Signed up": (a: schema.EUserInfo, b: schema.EUserInfo) =>
      b.signupTimestamp - a.signupTimestamp,
  ...
  constructor(app: AdminView) {
    ...
    buttonBox
      .eadd("<button>New User</button>")
      .on("click", async (e) => await this.startNewUser());
    buttonBox
      .eadd("<button>Assign Tasks</button>")
      .on("click", async (e) => await this.startBulkAssign());
    buttonBox
      .eadd("<button>Refresh</button>")
      .on("click", async (e) => await this.app.data.update());
  ...
  private drawTable() {
    ...
    for (const user of this.toSorted(this.app.data.users)) {
      ...
      tr.eadd("<td class=euid />")
        .eadd(`<a href="#/user/${euid}" />`)
        .etext(euid);
  ...
  private static getComparator(label: string) {
    if (label.startsWith("-")) {
